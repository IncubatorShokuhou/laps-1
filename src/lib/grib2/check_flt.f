      subroutine check_flt(kfildo,a,ib,nval,nxy,is5,ns5,iclean,ibitmap,
...
c        december 2001   glahn    added test for xmisss=xmissp;
c                                 added kfildo to call
c        february 2002   glahn    modified comment for nval
c
c        purpose
c            determines how many values are present a floating 
c            point data field based on the packing method and
c            whether or not a bit-map was provided to this routine.
c            this routine also determines the existence of primary
...
c
c        variables
c                a(k) = a( ) contains the data to be packed into 
c                       the grib message (k=1,nval).  (input/output)
...
c
      dimension a(nxy),ib(nxy),is5(ns5)
      dimension jer(ndjer,2)
c
      if(is5(23).eq.2)then
c
         if(xmisss.eq.xmissp)then
c              xmisss is equal to xmissp when both types of missings
...
c
      else if((is5(10).ge.2).and.(is5(23).eq.1))then
...
c
      else if((is5(10).ge.2).and.(is5(23).eq.2))then
c           the user must have supplied is5(23) when
c           is5(10).ge.2.
c
...
c
         do 40 k=1,nxy
            if(a(k).eq.xmissp)jmissp=.true.
            if(a(k).eq.xmisss)jmisss=.true.
